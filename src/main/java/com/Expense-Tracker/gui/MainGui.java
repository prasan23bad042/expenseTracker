...
class CategoryGui extends JFrame{
    ...
    private void initializeComponents(){
        ...
        categoryTable.setSelectionMode(ListSelectionModel.SINGLE_SELECTION); //Single row can be selected at a time
        categoryTable.getSelectionModel().addListSelectionListener( 
            e->{
            if(!e.getValueIsAdjusting()){   // runs once after selection is finalized
                loadSelectedCategory();     // method that loads the selected row’s details into the text fields
            }
        });
    }
